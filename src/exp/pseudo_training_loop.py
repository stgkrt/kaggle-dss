...
from losses import get_pseudo_criterion
...
def get_pseudo_target_key_list(series_df: pd.DataFrame) -> list:
    if "is_pseudo_target" not in series_df.columns:
        series_df["pseudo_count"] = series_df["event"].apply(
            lambda x: int(x == -1))
        series_df["pseudo_count"] = series_df.groupby(
            "series_date_key")["pseudo_count"].transform("sum")
        # series_date_keyごとにpseudo_countが0以上のところを1にする
        series_df["is_pseudo_target"] = series_df["event"].apply(
            lambda x: int(x > 0))
        series_df["is_pseudo_target"] = series_df.groupby(
            "series_date_key")["is_pseudo_target"].transform("sum")
        series_df["is_pseudo_target"] = (series_df["is_pseudo_target"]
                                         > 0).astype("uint8")
    pseudo_target_key_list = series_df[series_df["is_pseudo_target"] >
                                       0]["series_date_key"].unique()
    return pseudo_target_key_list


# TODO:loaderからtargetとpsudo targetを取り出すように変更する
def pseudo_train_fn(CFG, epoch, model, train_loader, criterion, optimizer,
                    LOGGER):
    model.train()
    ...
    losses = AverageMeter()
    for batch_idx, (inputs, targets, pseudo_targets,
        ...
        pseudo_targets = pseudo_targets.to(CFG.device,
...
def set_pseudo_label(CFG, LOGGER, model, series_df, key_df):
    ...
    pseudo_series_df = series_df[series_df["series_date_key"].isin(
        pseudo_target_key_list)]
    pseudo_key_df = key_df[key_df["series_date_key"].isin(
        pseudo_target_key_list)]
    # get loader
    pseudo_loader = get_loader(CFG,
                               pseudo_key_df,
                               pseudo_series_df,
                               mode="test")
    # predict
...
def pseudo_training_loop(CFG, LOGGER):
    ...
    for fold in CFG.folds:
        ...
        train_key_df = key_df[key_df["series_id"].isin(
            train_series_df["series_id"].unique())]

        valid_series_df = series_df[series_df["fold"] == fold]
        valid_key_df = key_df[key_df["series_id"].isin(
            valid_series_df["series_id"].unique())]
        ...
        for epoch in range(0, CFG.n_epoch):
            ...
            train_series_df = set_pseudo_label(CFG, LOGGER, model, series_df,
                                               key_df)
            # pseudo labeling
        ...
        oof_df_fold_path = os.path.join(oof_df_fold_dir,
                                        f"oof_df_pseudo_fold{fold}.parquet")
