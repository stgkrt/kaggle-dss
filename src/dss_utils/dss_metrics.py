...
def score(
    ...
    ...     'time': [10, 20, 40],
    ... })
    >>> score(solution, submission, tolerances,
        **column_names, use_scoring_intervals=True)
    1.0
...
def filter_detections(detections: pd.DataFrame,
    ...
    detection_time = detections.loc[:,
                                    time_column_name].sort_values().to_numpy()
...
def match_detections(tolerance: float, ground_truths: pd.DataFrame,
    ...
    a common event x tolerance x series_id evaluation group"""
    ...
    is_matched = np.full_like(detections_sorted[event_column_name],
                              ...
                              dtype=bool)
    gts_matched = set()
    detections_sorted_values = detections_sorted[time_column_name].values
    for i, det in enumerate(detections_sorted_values):
        # for i, det in enumerate(detections_sorted.itertuples(index=False)):
        best_error = tolerance
        best_gt = None
        for gt in ground_truths.itertuples(index=False):
            error = abs(det) - getattr(gt, time_column_name)
...
def precision_recall_curve(
        ...
        p: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:  # t
    if len(matches) == 0:
...
def event_detection_ap(
    ...
    if use_scoring_intervals:
        intervals = (solution.query("event in ['start', 'end']").assign(
            interval=lambda x: x.groupby(
                [series_id_column_name, event_column_name]).cumcount()).pivot(
                    index="interval",
                    columns=[series_id_column_name, event_column_name],
                    values=time_column_name,
                ).stack(series_id_column_name).swaplevel().sort_index().
                     loc[:, ["start", "end"]].apply(
    ...
    # with a column indicating a match to a ground-truth event
