...
def set_train_groupby_label(train_series_: pd.DataFrame,
                            train_event_: pd.DataFrame) -> pd.DataFrame:
    # abs diffにつかうaverage pool
    print("set unknown_onset and unknown_wakeup for null step")
    train_event_.loc[(train_event_["step"].isnull()) &
                     (train_event_["event"] == "onset"),
                     "event"] = "unknown_onset"
    train_event_.loc[(train_event_["step"].isnull()) &
                     (train_event_["event"] == "wakeup"),
                     "event"] = "unknown_wakeup"
...
def preprocess_train_series(train_series_: pd.DataFrame,
                            train_event_: pd.DataFrame) -> pd.DataFrame:
    print("set unknown_onset and unknown_wakeup for null step")
...
if __name__ == "__main__":
    ...
    train_event_df["series_date_key"] = train_event_df["series_id"].astype(
        str) + "_" + train_event_df["date"].astype(str)
    ...
    train_series_df.to_parquet(
        "/kaggle/input/targetdownsample_train_series_event.parquet")
