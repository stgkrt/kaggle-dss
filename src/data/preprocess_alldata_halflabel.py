...
def set_train_groupby_label(
    ...
    train_event_["event"] = train_event_["event"].map(
        {"onset": 0, "wakeup": 1, "unknown_onset": 0.5, "unknown_wakeup": 0.5}
    )
    train_event_["event"] = train_event_["event"].fillna(0.5)
...
if __name__ == "__main__":
    ...
    print(len(train_series_df))
    train_series_df.to_parquet("/kaggle/input/train_series_alldata_halflabel.parquet")
