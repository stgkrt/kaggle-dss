...
#         train_series_["series_id"].astype(str) +
# "_" + train_series_["date"].astype(str)
#     )
#     return train_series_
...
#  = train_series_["series_date_key"].astype(str)
#     train_series_["series_date_key"] = le.fit_transform(
#         train_series_["series_date_key_str"]
#     )
#     train_series_["series_date_key"]
# = train_series_["series_date_key"].astype("int16")
#     return train_series_
...
#     df, N=301, maxpool_kernel_size=41, maxpool_stride=1
# ) -> pd.DataFrame:
#     # series_idでgroupbyして、class_predに対して対象の列のデータから前のN個の列までのデータの平均をとる
#     df["class_pred"] = df["class_pred"].astype(np.float16)
#     df["class_pred_beforemean"] = (
#         df.groupby("series_id")["class_pred"]
#         .apply(lambda x: x.rolling(N, min_periods=1).mean())
#         .reset_index(drop=True)
#     )
#     df["class_pred_aftermean"] = (
#         df.groupby("series_id")["class_pred"]
#         .apply(lambda x: x[::-1].rolling(N, min_periods=1).mean()[::-1])
#         .reset_index(drop=True)
#     )
...
#         key_df = series_df[["series_date_key", "series_date_key_str"]]
# .drop_duplicates()
...
#         key_df["series_id"], key_df["date"] = key_df["series_date_key_str"].str.split(
#             "_", expand=True
#         )
...
#         ].mean(axis=1)
#         pred_df = pred_df
# .drop(columns=[f"class_pred_fold{fold}" for fold in CFG.folds])
...
#     for idx in range(split_num):
#         if idx == 0:
#             sub_df = pd.read_csv(os.path.join(tmp_file_path,
...
#     # series_df_path = (
#     #     "/kaggle/input/
# child-mind-institute-detect-sleep-states/train_series.parquet"
#     # )
